...
const Notes = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const profoId = localStorage.getItem('profoId');

        if (!profoId) {
          throw new Error('Professor ID not found in local storage');
        }

        console.log(`Fetching notes for professor ID: ${profoId}`);

        // Fetch module IDs associated with the professor
        const profDetailsResponse = await axios.get(`http://localhost:8080/api/profs/${profoId}`);
        if (profDetailsResponse.status !== 200) {
          throw new Error('Failed to fetch professor details');
        }
        const modIds = profDetailsResponse.data.modIds;

        console.log(`Module IDs associated with professor:`, modIds);

        // Fetch notes for each module taught by the professor
