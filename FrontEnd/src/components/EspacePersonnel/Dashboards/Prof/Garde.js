...
const Emplois = () => {
  ...
  useEffect(() => {
    const fetchEmploiDuTemps = async () => {
      try {
        const profId = parseInt(localStorage.getItem('profoId'), 10);
        ...
        // Filter seances by profId and seanceType = "EXAMS"
        console.log('profId:', profId);
        const filteredSeances = seances.filter(seance => seance.profId === profId && seance.seanceType === "EXAMS");
