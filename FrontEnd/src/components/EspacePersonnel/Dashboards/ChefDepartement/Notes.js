...
import ChefDDrawer from '../../Drawers/ChefDepDrawer';
import {
  Table,
  TableBody,
  TableCell,
...
const Notes = () => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const chefdId = localStorage.getItem('chefd');

        if (!chefdId) {
          throw new Error('Chef department ID not found in local storage');
        }

        console.log(`Fetching notes for chef department ID: ${chefdId}`);

        // Fetch module IDs associated with the chef department
        const chefDeptDetailsResponse = await axios.get(`http://localhost:8080/api/chefdepartements/${chefdId}`);
        if (chefDeptDetailsResponse.status !== 200) {
          throw new Error('Failed to fetch chef department details');
        }
        const modIds = chefDeptDetailsResponse.data.modIds;

        console.log(`Module IDs associated with chef department:`, modIds);

        // Fetch notes for each module associated with the chef department
